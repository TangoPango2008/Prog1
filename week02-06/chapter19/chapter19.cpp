...
template<typename T>
struct S{
	S(T vv = 0) : val{vv} {} //default 0 lesz, maskepp vv
...
S<T>& S<T>::operator =(const T& s){
	val = s;
	return *this; //ha sikeres akkor az s obj. adja vissza
}
...
int main(){
	...
	S<std::vector<int>> svi {std::vector<int>{1,1,2,3,5,8}}; //s tipusu obj ami int-eket tartalmaz
